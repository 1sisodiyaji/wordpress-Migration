...
class Site_Scanner {
	...
	public function routes() {
		...
		// Block / classic themes often render `/` from front-page.html with no static page.
		if ( ! $has_home ) {
			array_unshift(
				$routes,
				array(
					...
					'type'         => 'home',
					'postType'     => 'page',
					'title'        => get_bloginfo( 'name' ) ? get_bloginfo( 'name' ) : 'Home',
					'status'       => 'publish',
					'pageBuilder'  => function_exists( 'wp_is_block_theme' ) && wp_is_block_theme() ? 'gutenberg' : $this->detect_builder(),
					'template'     => 'front-page',
					'parentId'     => null,
					'menuOrder'    => 0,
					'isFront'      => true,
