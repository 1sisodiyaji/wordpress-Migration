...
import {
  ...
  storeWpConfigFile,
  storeWpContentZip,
} from "../../Converter/shared/wp-import/store-parts";
...
export function registerUploadRoutes(app: Express): void {
  app.post(
    ...
    (req, res) => {
      ...
      pipelineBanner(`UPLOAD (WP files) — ${slug}`, slug);
      pipelineDetail("importDir", importDir, slug);
      ...
      if (sqlFile) {
        ...
        pipelineStep(
          "upload",
          `Saved SQL → ${path.join(importDir, sqlFile.originalname)} (${sqlFile.size} bytes)`,
          slug,
        );
      ...
      if (wpContentFile) {
        ...
        pipelineStep(
          "upload",
          `Received wp-content zip: ${wpContentFile.originalname} (${wpContentFile.size} bytes)`,
          slug,
        );
        storeWpContentZip(importDir, wpContentFile.buffer);
        pipelineStep("unzip", `Unzipped wp-content → ${importDir}`, slug);
      ...
      if (wpConfigFile) {
        ...
        pipelineStep("upload", `Saved wp-config.php → ${importDir}`, slug);
      ...
      pipelineOk(`Upload saved. readyToImport=${status.readyToImport}`, slug);
      if (status.missing.length) pipelineDetail("missing", status.missing.join(", "), slug);
  ...
  app.post(
    ...
    async (req, res, next) => {
      try {
        ...
        pipelineBanner(`UPLOAD (plugin ZIP) — ${slug}`, slug);
        pipelineStep("upload", `Saved ${file.originalname} (${file.size} bytes)`, slug);
        pipelineDetail("zipPath", zipPath, slug);
        ...
        pipelineOk(`Plugin export imported for ${slug}`, slug);
