...
import { wpHttpFetch } from "../../lib/wp/http";
import { getMigratedPublicDir } from "../../lib/wp/config";
import { normalizeWordPressUrl, urlToSlug } from "../../lib/wp/sites";
import { detectBuilderFromHtml } from "../core/lib/html-extract";
import { extractLivePageAssets } from "../core/lib/live-assets";
import { bootstrapMigrateEnv } from "../core/bootstrap";
import { runMigration } from "../core/run";
...
async function migrateOne(url: string): Promise<SiteResult> {
  ...
  try {
    ...
    const { resetRestBaseCache } = await import("../../lib/wp/client");
    const { resetCssRegistry } = await import("../core/lib/css-download");
    const { resetJsRegistry } = await import("../core/lib/js-download");
    resetRestBaseCache();
    resetCssRegistry();
    resetJsRegistry();
