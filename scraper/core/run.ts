...
import { getMigratedDataDir, getWpUrl } from "../../lib/wp/config";
import { probeWordPressUrl } from "../../lib/wp/http";
import { appendMigrationLog } from "../../lib/wp/migration-log";
import { setMigrationPhase } from "../../lib/wp/migration-status";
import { normalizeWordPressUrl, upsertSite } from "../../lib/wp/sites";
...
async function prepareSourceUrl(slug?: string): Promise<string> {
  const normalized = normalizeWordPressUrl(getWpUrl());
  console.log(`  → Probing ${normalized}…`);
  try {
    const live = await probeWordPressUrl(normalized);
    if (live !== normalized) {
      console.log(`  ✓ Resolved URL: ${live}`);
    } else {
      console.log(`  ✓ Site reachable`);
    }
    process.env.WORDPRESS_URL = live;
    return live;
  } catch (err) {
    const detail = err instanceof Error ? err.message : String(err);
    if (slug) appendMigrationLog(slug, `[probe] ${detail}`);
    throw err;
  }
}
