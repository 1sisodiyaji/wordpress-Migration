...
import { setMigrationProgress } from "../../lib/wp/migration-status";
...
function crawlDelayMs(): number {
  ...
  try {
    const host = new URL(process.env.WORDPRESS_URL ?? "http://localhost:8082").hostname;
...
async function crawlRoute(
  ...
  const { getActiveSiteSlug } = await import("../../lib/wp/config");
  ...
  const { getWpUrl } = await import("../../lib/wp/config");
