...
// Prefer IPv4 — many WP hosts have broken/slow IPv6 and undici fails with "fetch failed".
...
const CONNECT_TIMEOUT_MS = Number(process.env.WORDPRESS_FETCH_CONNECT_MS ?? "45_000");
const HEADERS_TIMEOUT_MS = Number(process.env.WORDPRESS_FETCH_HEADERS_MS ?? "180_000");
const BODY_TIMEOUT_MS = Number(process.env.WORDPRESS_FETCH_BODY_MS ?? "300_000");
const DEFAULT_RETRIES = Number(process.env.WORDPRESS_FETCH_RETRIES ?? "8");
const RETRY_DELAY_MS = Number(process.env.WORDPRESS_FETCH_RETRY_MS ?? "3000");
const FORCE_IPV4 = process.env.WORDPRESS_FETCH_IPV4 !== "0";

function connectOptions() {
  return {
    timeout: CONNECT_TIMEOUT_MS,
    ...(FORCE_IPV4 ? { family: 4 as const } : {}),
  };
}

function createAgent(opts?: { allowH2?: boolean }): Agent {
  return new Agent({
...
// Rotates agents so retries don't reuse a bad connection.
function agentForAttempt(attempt: number): Agent {
  const useH2 = attempt % 2 === 0;
  return createAgent({ allowH2: useH2 });
}
...
export function isRetryableNetworkError(err: unknown): boolean {
  ...
  const msg = errorDetail(err).toLowerCase();
...
function formatNetworkError(url: string, err: Error, attempt: number, max: number): string {
  return [
    `Network error fetching ${url} (attempt ${attempt}/${max}): ${errorDetail(err)}`,
    "The WordPress host closed the connection or timed out.",
    "Common causes: LiteSpeed rate limits, slow TTFB, IPv6 issues, missing trailing slash on subdirectory installs.",
    "Try:",
    "  • Ensure URL ends with / for subdirectory WP (e.g. …/smartco/)",
    "  • Set WORDPRESS_FETCH_RETRIES=10  WORDPRESS_FETCH_BODY_MS=300000",
    "  • Set WORDPRESS_COOKIE if the site uses Cloudflare",
    "  • Use Files / WP export (.zip) if live scrape keeps failing",
  ].join("\n");
}

type FetchMode = "agent" | "plain";
...
async function rawFetch(
  ...
  return undiciFetch(url, {
    ...requestInit,
    dispatcher: agentForAttempt(attempt),
...
async function attemptFetch(
  ...
  const mode: FetchMode = attempt >= retries - 1 ? "plain" : "agent";
  const res = await rawFetch(url, init, attempt, mode);
...
function scheduleRetry(
  ...
  const delay = RETRY_DELAY_MS * (attempt + 1);
  console.warn(
    `  ↻ ${errorDetail(err)} — retry ${attempt + 2}/${retries + 1} in ${delay}ms (${url})`,
  );
  return new Promise((r) => setTimeout(r, delay));
}

/** HEAD/GET probe — follows redirects and returns the final reachable URL. */
export async function probeWordPressUrl(url: string): Promise<string> {
  const response = await wpHttpFetch(url, { method: "GET" });
