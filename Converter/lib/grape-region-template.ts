...
interface Props {
  pageKey: string;
  /** Optional fallback while disk content loads. Disk/API is source of truth. */
  initialHtml?: string;
}

/** Elementor-aligned breakpoints (max-width media queries). */
const DEVICES = [
  { id: "desktop", name: "Desktop", width: "", widthMedia: "", Icon: Monitor },
  { id: "laptop", name: "Laptop", width: "1366px", widthMedia: "1366px", Icon: Laptop },
  { id: "tablet", name: "Tablet", width: "768px", widthMedia: "1024px", Icon: Tablet },
  { id: "mobilePortrait", name: "Mobile", width: "375px", widthMedia: "767px", Icon: Smartphone },
] as const;
...
/** Remove stale browser-only drafts so every profile loads from disk. */
function clearLocalGrapeDrafts(pageKey: string): void {
  try {
    const needle = grapeStorageKey(pageKey);
    const remove: string[] = [];
    for (let i = 0; i < localStorage.length; i++) {
      const k = localStorage.key(i);
      if (k && (k.includes(needle) || k.includes(\`-\${pageKey}\`))) remove.push(k);
    }
...
async function fetchPageFromDisk(pageKey: string): Promise<DiskPagePayload> {
  const res = await fetch(\`/api/grape/page?pageKey=\${encodeURIComponent(pageKey)}\`, {
    cache: "no-store",
  });
  const data = (await res.json().catch(() => null)) as
    | (DiskPagePayload & { ok?: boolean; error?: string })
    | null;
  if (!res.ok || !data?.ok) {
    throw new Error(data?.error ?? \`Failed to load page from disk (HTTP \${res.status})\`);
  }
  return {
    contentHtml: data.contentHtml ?? "",
    grapeBlocks: data.grapeBlocks ?? null,
    contentMode: data.contentMode ?? "html",
    canvasStyles: data.canvasStyles ?? [],
    canvasScripts: data.canvasScripts ?? [],
  };
}
...
/** Inner HTML of a GrapesJS component (children only). */
function componentInnerHtml(comp: { components: () => { models: Array<{ toHTML: () => string }> } }): string {
  try {
    return comp.components().models.map((m) => m.toHTML()).join("");
  } catch {
    return "";
  }
}

function findLayoutComponent(editor: Editor, type: string, attr: string) {
  const wrapper = editor.getWrapper();
  if (!wrapper) return null;
  try {
    const byType = wrapper.findType(type);
    if (byType?.[0]) return byType[0];
  } catch {
    /* ignore */
  }
  try {
    const byAttr = wrapper.find(\`[data-layout="\${attr}"]\`);
...
/** Extract page body / header / footer for filesystem persistence. */
function extractPersistPayload(editor: Editor, preferBlocks: boolean) {
  const bodyComp = findLayoutComponent(editor, "page-body", "body");
  const headerComp = findLayoutComponent(editor, "site-header", "header");
  const footerComp = findLayoutComponent(editor, "site-footer", "footer");

  let html = "";
  let grapeBlocks: unknown[] | null = null;

  if (bodyComp) {
    html = componentInnerHtml(bodyComp);
    if (preferBlocks) {
      try {
        grapeBlocks = bodyComp
          .components()
          .models.map((c: { toJSON: () => unknown }) => c.toJSON());
      } catch {
        grapeBlocks = null;
      }
    }
  } else {
    const full = editor.getHtml() || "";
    const match =
      full.match(/<main[^>]*data-layout="body"[^>]*>([\\s\\S]*?)<\\/main>/i) ||
      full.match(/<div[^>]*data-layout="body"[^>]*>([\\s\\S]*?)<\\/div>/i);
    html = match?.[1]?.trim() ?? full;
  }

  const headerHtml = headerComp ? componentInnerHtml(headerComp) : null;
  const footerHtml = footerComp ? componentInnerHtml(footerComp) : null;
  const css = editor.getCss() || "";

  return { html, css, grapeBlocks, headerHtml, footerHtml };
}

async function persistPageToDisk(
  pageKey: string,
  payload: {
    html: string;
    css: string;
...
export function GrapeRegion({ pageKey, initialHtml = "" }: Props) {
  ...
  const [diskHtml, setDiskHtml] = useState(initialHtml);
  const [diskBlocks, setDiskBlocks] = useState<unknown[] | null>(null);
  const [diskMode, setDiskMode] = useState("html");
  const [diskStyles, setDiskStyles] = useState<string[]>([]);
  const [diskScripts, setDiskScripts] = useState<string[]>([]);
  const contentMode = diskMode || "html";

  useEffect(() => {
    let cancelled = false;
    setDiskReady(false);
    clearLocalGrapeDrafts(pageKey);
    fetchPageFromDisk(pageKey)
      .then((page) => {
        if (cancelled) return;
        setDiskHtml(page.contentHtml);
        setDiskBlocks(page.grapeBlocks);
        setDiskMode(page.contentMode);
        setDiskStyles(page.canvasStyles);
        setDiskScripts(page.canvasScripts);
        setDiskReady(true);
      })
      .catch(() => {
        if (cancelled) return;
        const pageMeta = siteData.pages.find((p) => p.key === pageKey);
        setDiskHtml(initialHtml || pageMeta?.contentHtml || "");
        setDiskBlocks((pageMeta as { grapeBlocks?: unknown[] | null } | undefined)?.grapeBlocks ?? null);
        setDiskMode(pageMeta?.contentMode ?? "html");
        setDiskStyles(pageMeta?.canvasStyles ?? siteData.canvasStyles ?? []);
        setDiskScripts(pageMeta?.canvasScripts ?? siteData.canvasScripts ?? []);
        setDiskReady(true);
      });
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    const canvasStyles = diskStyles;
    const canvasScripts = diskScripts;
    const grapeBlocks = diskBlocks;
    const bodyHtml = usableBodyHtml(diskHtml);
    ...
    const schedulePersist = () => {
      ...
      saveTimerRef.current = window.setTimeout(async () => {
        ...
        try {
          const payload = extractPersistPayload(ed, contentMode === "blocks");
          await persistPageToDisk(pageKey, payload);
          clearLocalGrapeDrafts(pageKey);
    ...
    return () => {
      ...
      if (saveTimerRef.current) window.clearTimeout(saveTimerRef.current);
      editor.off("update", schedulePersist);
      editor.destroy();
      editorRef.current = null;
      shell.classList.remove("grape-ready", "is-preview");
      setIsPreview(false);
    };
  }, [pageKey, diskReady, diskHtml, diskBlocks, diskMode, diskStyles, diskScripts, contentMode]);
          ...
          <div className="grape-canvas-wrap">
            <header className="grape-toolbar grape-toolbar--float">
              ...
              <div className="grape-toolbar-meta">
                <span
                  className={\`grape-save-badge grape-save-badge--\${saveStatus}\`}
                  title={saveError ?? "Edits autosave to src/data/site.json"}
                >
                  {saveStatus === "pending" && "Unsaved…"}
                  {saveStatus === "saving" && "Saving…"}
                  {saveStatus === "saved" && "Saved to disk"}
                  {saveStatus === "error" && "Save failed"}
                  {saveStatus === "idle" && "Live sync"}
                </span>
                <button
                  type="button"
...
.grape-save-badge--pending { color: #fbbf24; }
.grape-save-badge--saving { color: #67e8f9; }
.grape-save-badge--saved { color: #6ee7b7; }
.grape-save-badge--error { color: #fca5a5; }
