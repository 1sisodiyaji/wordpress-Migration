...
export function cleanGeneratedPublic(projectDir: string): void {
  ...
  for (const name of fs.readdirSync(publicDir)) {
    if (PRESERVED_PUBLIC_ENTRIES.has(name)) continue;
    rmPathSafe(path.join(publicDir, name));
  }

  // Remove recursive assets/assets/... nests left by older buggy copies.
  const assetsDir = path.join(publicDir, "assets");
  if (fs.existsSync(assetsDir)) {
    rmPathSafe(assetsDir);
  }
}
...
export function cleanGeneratedProject(projectDir: string): void {
  ...
  for (const entry of ["src", "index.html", "vite.config.ts", "tsconfig.json", "package.json"]) {
    rmPathSafe(path.join(projectDir, entry));
  }
  cleanGeneratedPublic(projectDir);
}
