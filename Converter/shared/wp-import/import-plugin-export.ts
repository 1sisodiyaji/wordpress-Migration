...
import { pipelineDetail, pipelineStep } from "../pipeline-log";
...
/**
 * Import a wp-grape-export bundle (folder or .zip) into Projects/{slug}/data/.
 *
 * Produces:
...
export async function importPluginExport(opts: {
  ...
  pipelineStep("unzip", `Opening bundle: ${opts.source}`, slug);
  const bundle = readPluginExportBundle(opts.source);
  pipelineDetail("bundleRoot", bundle.root, slug);
  pipelineDetail("isTempExtract", String(bundle.isTemp), slug);
  try {
...
function landBundle(
  ...
  pipelineStep("import", `Landing bundle → Projects/${slug}/`, slug);
  pipelineDetail("dataDir", dataDir, slug);
  pipelineDetail("publicDir", publicDir, slug);
  pipelineDetail("routes", String(routes.length), slug);
  ...
  if (fs.existsSync(bundleInline)) {
    ...
    assetsCopied = true;
    cssJsCopied = true;
    pipelineStep("sync", `Synced inline assets → ${dest}`, slug);
  }
